...
from .forms import NewItemForm, EditItemForm, BuyItemForm
...
def buy_ticket(request, pk, ):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ticket = Ticket(item=item, owner=request.user)
            ticket.save()
            return HttpResponseRedirect('/items/' + str(item.id ) + '/buy?submitted=true')
    else:
        form = BuyItemForm
